...
| The site could not send mail at all | `backend/app/mail.py`: an SMTP sender that **never raises**, 10 s timeout, and a `configured()` predicate so a development machine simply does not send rather than failing a signup. It does not shell out to the host's `msmtp` — it cannot, being an unprivileged container — so it speaks SMTP to the same relay. **It sends as the CLUB's own mailbox** — `club_mail_host`, `club_mail_port`, `club_mail_from` and `club_mail_user` in `params.conf`, with the password read on the server out of the root-only file `club_mail_password_file` names — and deliberately **not** as the machine's. It borrowed the machine's msmtp identity first, and the consequence was a member asked to confirm their address being written to by `spatzengamingserver@kw-it.info`: a machine account at a domain they have never heard of, which is indistinguishable from spam. The machine's own operations mail is unchanged and still reaches whoever runs the box — one failing must not silence the other. |
