...
| Static pages (Platz, Mitgliedschaft, Kontakt, Impressum, Datenschutz, Rechtliches) | `App.tsx` + backend Markdown | done |
...
| Members: signup *and* Google, welcome page | `Members.tsx` + `api.ts` | **frontend only — `/api/auth/*` is not written** |
